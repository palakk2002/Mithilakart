...
const ContinueShopping = () => {
  ...
  return (
    <div className="bg-[#f4faf6] min-h-screen pb-24 font-sans text-slate-800">
      {/* Sticky Header */}
      <header className="sticky top-0 z-50 bg-[#3E5A44] text-white px-4 py-3 flex items-center justify-between shadow-md">
        ...
        <div className="flex items-center gap-3.5">
          ...
          >
            ...
            <span className="absolute -top-0.5 -right-0.5 bg-[#e2a750] text-slate-900 text-[8px] font-black w-4 h-4 rounded-full flex items-center justify-center border border-[#3E5A44]">
              2
            </span>
          </div>
        </div>
      ...
      <div className="px-4 py-4 space-y-6">
            ...
            {[clickedProduct, relatedRecentlyViewed].map((product, idx) => (
              ...
              >
                <div className="aspect-square m-1.5 rounded-xl overflow-hidden relative bg-slate-50 border border-slate-100/55 flex items-center justify-center">
                  ...
                  {idx === 0 && (
                    <div className="absolute top-1.5 left-1.5 bg-[#3E5A44]/90 backdrop-blur-xs text-white text-[7.5px] font-black px-1.5 py-0.5 rounded-full tracking-wider uppercase">
                      Last Viewed
                    </div>
                  )}
                </div>
            ...
            {filters.map((filter, idx) => (
              <button 
                ...
                className={`whitespace-nowrap px-4 py-1.5 rounded-full text-[11px] font-black border transition-all ${
                  activeFilter === filter 
                    ? 'bg-[#3E5A44] border-[#3E5A44] text-white shadow-xs' 
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-3">
            {['#Gingham', '#Oversized', '#SummerStripe', '#RetroCheck', '#PastelMix'].map((trend, idx) => (
              <button 
                ...
                className={`whitespace-nowrap px-4 py-1.5 rounded-full text-[11px] font-black border transition-all ${
                  idx === 0 
                    ? 'bg-emerald-50 border-emerald-200 text-[#3E5A44] shadow-xs' 
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="text-[10px] font-black text-[#3E5A44] tracking-widest uppercase hover:underline">
              View all
            </button>
          </div>
