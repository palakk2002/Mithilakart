    ...
    <div className="min-h-screen bg-[#f8fafc] pb-24 font-sans">
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-slate-100 px-4 py-4 flex items-center justify-between">
        ...
        <button 
          ...
          className={`p-2 rounded-xl transition-colors ${isEditing ? 'bg-blue-50 text-[#6FAE4A]' : 'text-slate-400'}`}
        >
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex flex-col items-center py-4">
          <div className="relative">
            <div className="w-20 h-20 bg-blue-100 rounded-full flex items-center justify-center border-4 border-white shadow-md">
              <User size={40} className="text-[#6FAE4A]" />
            </div>
            {isEditing && (
              <button className="absolute bottom-0 right-0 p-2 bg-[#6FAE4A] text-white rounded-full border-2 border-white shadow-lg animate-in zoom-in duration-300">
                <Camera size={14} />
              </button>
            )}
          </div>
              ...
              {[
                ...
                <div key={field.key} className={`bg-white rounded-2xl p-4 border transition-all ${isEditing ? 'border-blue-200 shadow-blue-50' : 'border-slate-100 shadow-sm'}`}>
                  ...
                  <div className="flex items-center gap-3">
                    <field.icon size={16} className={`${isEditing ? 'text-[#6FAE4A]' : 'text-slate-400'}`} />
                    <input 
                      type="text" 
                      readOnly={!isEditing}
        ...
        {isEditing && (
          <motion.button 
            ...
            onClick={handleSave}
            className="w-full bg-[#6FAE4A] text-white py-4 rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-blue-100 active:scale-95 transition-all flex items-center justify-center gap-2 mt-8"
          >
            <Save size={18} />
            Save Changes
          </motion.button>
